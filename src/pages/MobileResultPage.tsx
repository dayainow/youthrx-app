...
const MobileResultContent = ({
  ...
  const saved = useResultImage(
    prescriptionRef,
    `마음약국_${main}_${prescription.issuedOn}.png`,
  );
  ...
  const getIngredientData = () => {
    ...
    const total =
      Object.values(prescription.scores).reduce((a, b) => a + b, 0) || 1;
    return (
      Object.keys(prescription.scores) as (keyof typeof prescription.scores)[]
    )
      ...
      <div className="result-page-frame bg-[#FAF8F2] text-gray-900 w-full shadow-xl relative overflow-y-auto flex flex-col sm:rounded-3xl">
        ...
        <div className="p-4 sm:p-6 flex-1 flex flex-col relative z-20 h-full min-h-0">
            ...
            >
              ...
              <div className="bg-[#FAF8F2] text-[#111] shadow-[0_20px_40px_rgba(0,0,0,0.12)] border-x border-b border-[#E8E1D5] rounded-b-md p-6 relative overflow-hidden">
                ...
                <div className="text-center mb-8 relative">
                  ...
                  <h2 className="text-3xl font-bold tracking-tight text-[#3E3A39] mb-2 font-serif">
                    마음약국
                  </h2>
                  <p className="text-sm text-[#7F8C8D] tracking-wide">
                    당신의 마음을 처방합니다
                  </p>
                </div>

                <p className="text-center text-sm font-bold text-[#8B4513] mb-5 break-keep">
                  {prescription.pillEmoji} {prescription.pillName}
                </p>

                {/* Patient Info */}
                <div className="flex justify-between items-end border-b-2 border-[#E8E1D5] pb-2 mb-6 px-1">
                  <div className="flex items-baseline space-x-2">
                    <span className="text-lg font-bold text-[#3E3A39]">
                      {main} 처방
                    </span>
                    <span className="text-xs text-[#7F8C8D]">귀하</span>
                  </div>
                  <span className="text-xs font-medium text-[#7F8C8D]">
                    {dateStr}
                  </span>
                </div>
                ...
                <div className="mb-8 bg-white border border-[#E8E1D5] rounded-2xl p-5 shadow-[0_2px_10px_rgba(0,0,0,0.02)]">
                  <h3 className="font-bold text-[14px] text-[#3E3A39] mb-4 flex items-center tracking-tight">
                    <span className="mr-2 text-lg">🧪</span> 맞춤 처방약 성분
                    분석표
                  </h3>
                ...
                {/* Prescription List */}
                <div className="mb-2">
                  <h3 className="font-bold text-lg text-[#3E3A39] mb-4 px-1">
                    처방 내역
                  </h3>
                  <div className="space-y-3">
                    {policies.map((policy, idx) => (
                      <div
                        key={policy.id}
                        className="block bg-white border border-[#E8E1D5] rounded-2xl p-4 shadow-sm group"
                      >
                        <div className="flex items-center">
                          ...
                          <div className="flex-1">
                            ...
                            <div className="text-[16px] font-extrabold text-[#3E3A39] leading-tight mb-1.5">
                              {policy.title}
                            </div>
                ...
                <div className="mt-4 p-4 bg-white rounded-xl border border-dashed border-[#D8CFC0]">
                  <div className="text-[12px] font-bold text-[#8B4513] mb-1.5 flex items-center">
                    <span className="mr-1.5 text-sm">🧾</span> 더 많은 도움이
                    필요하다면?
                  </div>
                  <div className="text-[13px] font-medium text-[#555] leading-relaxed break-keep">
                    선택하신 분야 외에도 서울청년센터 마포에서 주거, 일자리,
                    금융, 심리 등 다양한 청년정책 종합상담을 받아보실 수 있어요.
                  </div>
                </div>
          ...
          <div className="text-center text-sm text-[#7F8C8D] mb-3">
            <button
              className="min-h-11 underline underline-offset-4 text-[#8B4513]"
              onClick={saved.copyLink}
            >
              결과 링크 복사
            </button>
            <p role="status" aria-live="polite" className="leading-relaxed">
              {saved.message}
            </p>
            {saved.showLink && (
              <input
                className="border border-[#D8CFC0] rounded-lg p-3 w-full mt-2"
                aria-label="내 결과 링크"
                readOnly
            ...
            {saved.error && (
              <div role="alert">
                <p>
                  이미지를 만들지 못했어요. 화면을 캡처하거나 링크를 보관해
                  주세요.
                </p>
                <button
                  className="min-h-11 underline text-[#8B4513]"
                  onClick={saved.retry}
                >
                  이미지 다시 만들기
                </button>
            ...
            {saved.showPreview && saved.image && (
              <details open className="mt-3">
                <summary className="min-h-11 cursor-pointer">
                  저장이 안 되면 이미지를 길게 눌러주세요
                </summary>
                <img
                  src={saved.image.url}
                  alt="저장용 마음약국 처방전"
...
export function MobileResultPage() {
  ...
  if (!prescription)
    return (
      <main className="min-h-screen bg-[#FAF8F2] flex flex-col items-center justify-center p-6 text-center text-[#3E3A39]">
        <span className="text-5xl" aria-hidden="true">
          ✉️
        </span>
        <h1 className="text-2xl font-bold mt-6">처방전을 확인할 수 없어요</h1>
        <p className="mt-4 text-[#7F8C8D] leading-relaxed">
          QR 주소가 잘렸거나 이전 버전의 결과입니다.
          <br />
          부스 화면의 QR을 다시 찍어주세요.
        </p>
        <a className="enhance-button enhance-primary mt-6" href="/">
          새 처방 받기
        </a>
      </main>
    );
